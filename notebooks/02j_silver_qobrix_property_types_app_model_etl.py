...
# MLS 2.0 - Bronze (qobrix_api_property_types) -> Silver (qobrix_silver_property_types)
#
# Purpose: Transform property type dictionary from bronze into normalized silver per app_data_model.dbml.
...
bronze_table = f"{catalog}.{schema}.qobrix_api_property_types"
...
transform_sql = f"""
CREATE OR REPLACE TABLE mls2.qobrix_silver_property_types AS
SELECT
    {col_or_null('id')},
    {trim_or_null('code')},
    {trim_or_null('name')},
    {trim_or_null('base_type')},
    {trim_or_null('config')},
...
print("Creating mls2.qobrix_silver_property_types from bronze...")
spark.sql(transform_sql)

silver_df = spark.table(f"{catalog}.{schema}.qobrix_silver_property_types")
print(f"✅ qobrix_silver_property_types: {silver_df.count()} rows, {len(silver_df.columns)} columns")
