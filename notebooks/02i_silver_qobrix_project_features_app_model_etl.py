...
# MLS 2.0 - Bronze (qobrix_api_project_features) -> Silver (qobrix_silver_project_features)
#
# Purpose: Transform project feature dictionary from bronze into normalized silver per app_data_model.dbml.
...
bronze_table = f"{catalog}.{schema}.qobrix_api_project_features"
...
transform_sql = f"""
CREATE OR REPLACE TABLE mls2.qobrix_silver_project_features AS
SELECT
    {col_or_null('id')},
    {trim_or_null('code')},
    {trim_or_null('label')},
...
print("Creating mls2.qobrix_silver_project_features from bronze...")
spark.sql(transform_sql)

silver_df = spark.table(f"{catalog}.{schema}.qobrix_silver_project_features")
print(f"✅ qobrix_silver_project_features: {silver_df.count()} rows, {len(silver_df.columns)} columns")
